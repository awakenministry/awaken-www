...
gulp.task('scripts', () =>
  ...
  gulp.src([
    ...
    paths.jsFiles + '/plugins/bootstrap.min.js',
    paths.jsFiles + '/plugins/respond.js',
    paths.jsFiles + '/plugins/kinetic.js',
    paths.jsFiles + '/plugins/jquery.final-countdown.js',
    paths.jsFiles + '/plugins/jquery.stellar.min.js',
    paths.jsFiles + '/plugins/jquery.plugin.js',
    paths.jsFiles + '/plugins/jquery.countdown.js',
    paths.jsFiles + '/plugins/jquery.bxslider.min.js',
    paths.jsFiles + '/plugins/jquery.content_slider.min.js',
    // paths.jsFiles + '/plugins/styleswitch.js',
    // paths.jsFiles + '/plugins/jquery.tabSlideOut.v1.3.js',
    paths.jsFiles + '/plugins/audioplayer.js',
    paths.jsFiles + '/plugins/TimeCircles.js',
    paths.jsFiles + '/custom.js'
  ])
...
gulp.task('styles:critical:page', function () {
  return gulp.src(paths.tempDir  + paths.siteDir + 'articles/ipad-pro/awaken-nights-paginator.html')
    .pipe(critical({
      base: paths.tempDir,
      inline: false,
...
gulp.task('styles:critical:archive', function () {
  return gulp.src(paths.tempDir  + paths.siteDir + 'mastering-paper/awaken-nights-paginator.html')
    .pipe(critical({
      base: paths.tempDir,
      inline: false,
      css: [paths.sourceDir + paths.includesFolderName + '/style.css'],
...
gulp.task('styles:critical:work', function () {
  return gulp.src(paths.tempDir  + paths.siteDir + 'paperfaces/asja-k-portrait/awaken-nights-paginator.html')
    .pipe(critical({
      base: paths.tempDir,
      inline: false,
      css: [paths.sourceDir + paths.includesFolderName + '/style.css'],
